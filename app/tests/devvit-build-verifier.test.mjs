...
test('production verifier requires shipped music and enforces audio budgets', () => {
  assert.match(verifierSource, /pocketful-of-ink\.mp3/);
  assert.doesNotMatch(verifierSource, /legends-in-the-margins\.mp3/);
  assert.doesNotMatch(verifierSource, /ready-set-scribble\.mp3/);
  assert.doesNotMatch(verifierSource, /scribbits-battle\.mp3/);
