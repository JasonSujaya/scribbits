...
const readClientSource = (relativePath) =>
  readFileSync(new URL(`../src/client/${relativePath}`, import.meta.url), 'utf8');
...
test('critical authored scenes check texture readiness and expose retry', () => {
  for (const [source, readyFunction, retryFunction, preloadFunction] of [
    [bootSource, 'coreVisualAssetsReady', 'retryVisualAssets', 'preloadVisualAssets'],
    [homeSource, 'homeVisualAssetsReady', 'retryHomeVisualAssets', 'preloadHomeVisualAssets'],
    [replaySource, 'replayVisualAssetsReady', 'retryReplayVisualAssets', 'preloadReplayVisualAssets'],
    [gallerySource, 'galleryVisualAssetsReady', 'retryGalleryVisualAssets', 'preloadGalleryVisualAssets'],
  ]) {
